            ...
            if i.key_pressed(egui::Key::Enter) {
                if self.ctrl_pressed {
                    ...
                    if let Some(index) = app.display_container_state.focused_index {
                        if index > 0
                            && (index - 1) < app.display_container_state.display_to_id.len()
                        {
                            ...
                            if !input_text.is_empty() {
                                execute_command(
                                    app,
                                    Some(Command::Edit(task_id, input_text.clone())),
                                );
                            }

                            // Find the nearest task at the same level before completing the task
                            let nearest_task_id = app
                                .display_container_state
                                .find_nearest_task_at_same_level(&app.tasks, task_id);
                            ...
                            app.display_container_state
                                .focus_task_and_update_input(nearest_task_id, &app.tasks);
                    ...
                    if let Some(index) = app.display_container_state.focused_index {
                        if index == 0 {
                            ...
                            if !input_text.is_empty() {
                                ...
                                app.display_container_state
                                    .focus_task_and_update_input(Some(new_task_id), &app.tasks);
                                // Update the input_text to match the display container's input value
                                *input_text = app.display_container_state.input_value().to_string();
                            }
                            ...
                            if !app.display_container_state.is_task_expanded(parent_id) {
                                app.display_container_state
                                    .toggle_task_expansion(parent_id, &app.tasks);
                            }
                            ...
                            if let Some(new_subtask_id) = subtask_id {
                                ...
                                if app
                                    .display_container_state
                                    .focus_task_and_update_input(Some(new_subtask_id), &app.tasks)
                                {
                                    // Update the input_text to match the display container's input value
                                    *input_text =
                    ...
                    if !input_text.is_empty() {
                        ...
                        match app.display_container_state.focused_index {
                            Some(0) | None => {
                                ...
                                app.display_container_state
                                    .focus_task_and_update_input(None, &app.tasks);
                            ...
                            Some(idx) => {
                                ...
                                if (idx - 1) < app.display_container_state.display_to_id.len() {
                                    let task_id =
                                        app.display_container_state.display_to_id[idx - 1];
                                    ...
                                    if let Some(original_idx) =
                                        app.display_container_state.original_focus
                                    {
                                        // Get the task ID for the original focus
                                        let original_task_id = if original_idx > 0
                                            && (original_idx - 1)
                                                < app.display_container_state.display_to_id.len()
                                        {
                                            Some(
                                                app.display_container_state.display_to_id
                                                    [original_idx - 1],
                                            )
                                        } else {
                                            None
                                        ...
                                        // Use the unified method to focus and update input
                                        if app.display_container_state.focus_task_and_update_input(
                                            original_task_id,
                                            &app.tasks,
                                        ) {
                                            // Update the input_text to match the display container's input value
                                        ...
                                        app.display_container_state
                                            .focus_task_and_update_input(Some(task_id), &app.tasks);
                                        // Update the input_text to match the display container's input value
                                        *input_text =
                                            app.display_container_state.input_value().to_string();
                                    }
                ...
                if i.key_pressed(egui::Key::ArrowUp) {
                    if let Some(index) = app.display_container_state.focused_index {
                        if index > 0 {
                            ...
                            if app
                                .display_container_state
                                .focus_task_and_update_input(task_id, &app.tasks)
                            {
                                // Update the input_text to match the display container's input value
                                *input_text = app.display_container_state.input_value().to_string();
                                handled = true;
                    ...
                    if let Some(index) = app.display_container_state.focused_index {
                        if index < app.display_container_state.display_to_id.len() {
                            ...
                            let task_id = if index == 0 {
                                ...
                                < app.display_container_state.display_to_id.len() - 1
                            {
                            ...
                            if let Some(task) = app.tasks.iter().find(|t| t.id == task_id) {
                                if !task.child_ids.is_empty() {
                                    ...
                                    if let Some(display_idx) =
                                        app.display_container_state.get_display_index(task_id)
                                    {
                                        app.log_activity(format!(
                                            "Toggled expansion of task {}",
                                            display_idx + 1
                                        )); // +1 because display indices are 1-based
                                    }
                                    handled = true;
                                }
                            }
