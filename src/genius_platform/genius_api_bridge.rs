...
use super::genius_api::{GeniusApiClient, GeniusApiError, GeniusItem, GeniusResponse};
use crate::App;
use serde_json;
use std::sync::Arc;
...
impl GeniusApiBridge {
    ...
    pub fn get_query_from_app(app: &App) -> String {
    ...
    pub async fn load_next_page(&self) -> Result<GeniusResponse, GeniusApiError> {
        ...
        if inner.current_query.is_empty() {
            return Err(GeniusApiError::Other(
                "No current query to load more results for".to_string(),
            ));
        }
    ...
    async fn execute_query_with_page(
        &self,
        query: &str,
        page: usize,
    ) -> Result<GeniusResponse, GeniusApiError> {
...
pub mod factory {
    ...
    pub fn create_mock_bridge() -> GeniusApiBridge {
        GeniusApiBridge::with_client(super::super::genius_api::mock::create_mock_client())
    }
...
mod tests {
    ...
    async fn test_basic_query() {
        ...
        for item in response.items {
            assert!(
                item.description.contains("test query"),
    ...
    async fn test_pagination() {
        ...
        assert_eq!(
            bridge.current_page().await,
            1,
            "Current page should be 1 after first query"
        );
        ...
        assert_eq!(
            current_page, 2,
            "Current page should be 2 after loading next page"
        );
        ...
        assert!(
            !items_after_second_query.is_empty(),
    ...
    async fn test_state_management() {
        ...
        assert!(
            bridge.all_items().await.is_empty(),
        ...
        assert_eq!(
            stored_response.unwrap().items.len(),
            5,
            "Should have 5 items in the stored response"
        );
    }
    ...
    async fn test_concurrent_access() {
        ...
        let task1 = tokio::spawn(async move { bridge.execute_query("concurrent test 1").await });

        // Spawn another task that makes a different query
        let task2 =
            tokio::spawn(async move { bridge_clone.execute_query("concurrent test 2").await });
        ...
        assert!(
            ...
            "First concurrent query should succeed"
        );
        assert!(
            result2.unwrap().is_ok(),
            "Second concurrent query should succeed"
        );
    }
}
